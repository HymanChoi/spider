...
function getTitles(page) {
  ...
  https.get(url + "?start=" + page + "&filter=", (res) => {
    ...
    res.on("end", () => {
      ...
      // 爬取中文标题
      $(".grid_view .item .info .title:nth-child(1)").each(function (
        index,
        element
      ) {
        let $e = $(element);
      ...
      // 爬取英文标题
      $(".grid_view .item .info .title:nth-child(2)").each(function (
        index,
        element
      ) {
        let $e = $(element);
      ...
      // // 爬取评分
      $(".grid_view .item .info .bd .star .rating_num").each(function (
        index,
        element
      ) {
        let $e = $(element);
...
function getInfo(Info, i) {
  ...
  https.get(arr[i].Link, (res) => {
    ...
    res.on("end", () => {
      ...
      $('#info [property$="v:initialReleaseDate"]').each(function (
        index,
        element
      ) {
        let $e = $(element);
      ...
      arr[i].InitialReleaseDate = initialReleaseDate.substring(
        0,
        initialReleaseDate.length - 3
      );
